    ...
    if (usedType) {
        ...
        for (let prp of lst) {
            if (prp != 'type') {
                ...
                if (prpObj.$ref) {
                    if (prpObj.$ref.startsWith('#/')) {
                        const linkName = prpObj.$ref.substring(2);
...
export class PropertyGrid extends BaseCustomWebComponentConstructorAppend {
    ...
    getTypeInfo = async (obj, type) => this.typeDefinitions.find(x => x.name == type);
    typeDefinitions;
    _table;
    _tree;
    _head;
    ...
    typeName;
    async createPropertyNodes(baseNode, properties, prefix = '') {
        if (!this.noCategory) {
    ...
    async createPropertyNodesInternal(baseNode, properties, prefix = '') {
    ...
    async getNullableWrapper(childControl, property, currentValue, propertyPath, wbRender, additionalInfo) {
    ...
    async getSimpleEditorForType(property, currentValue, propertyPath, wbRender, additionalInfo) {
