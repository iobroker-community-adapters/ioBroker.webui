            ...
            if (node.nodeType === 1) { //node.nodeType === 1
                ...
                for (let a of attributes) {
                    if (a.value.startsWith('[[') && a.value.endsWith(']]')) {
                        ...
                        b(true);
                        if (event) {
                            event.split(';').forEach(x => node.addEventListener(x, (e) => this._bindingsSetValue(host ?? this, value.replaceAll('?', ''), node[camelCased], context)));
                        }
                    }
                }
            }
                ...
                if (node.nodeValue.indexOf('[[') >= 0) {
                    ...
                    for (let m of matches) {
                        if ((m[0].length == trimmedLength || (m.index == 0 && m[0].length == text.length)) && node.parentNode.childNodes.length == 1) {
                            ...
                            const b = (firstRun) => this._bindingSetNodeValue(firstRun, parent, null, 'innerHTML', value, repeatBindingItems, removeAttributes, host, context, noNull);
                            this._bindings.push([b, null]);
                            b(true);
                        }
                        else {
                            ...
                            const b = (firstRun) => this._bindingSetNodeValue(firstRun, newNode, null, 'innerHTML', value, repeatBindingItems, removeAttributes, host, context, noNull);
                            this._bindings.push([b, null]);
                            b(true);
