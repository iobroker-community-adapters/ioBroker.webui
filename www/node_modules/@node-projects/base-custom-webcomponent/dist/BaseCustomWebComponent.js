...
export class BaseCustomWebComponentNoAttachedTemplate extends HTMLElement {
                ...
                for (let a of attributes) {
                    if (a.value[0] === '[' && a.value[1] === '[' && a.value[a.value.length - 1] === ']' && a.value[a.value.length - 2] === ']') {
                        if (a.name.startsWith('css:')) {
                            ...
                            const b = (firstRun) => this._bindingSetNodeValue(firstRun, node, a, camelCased, value, repeatBindingItems, removeAttributes, host, context, noNull);
                            this._bindings.push([b, null]);
                            b(true);
                        }
                    }
                        ...
                        let event = node instanceof HTMLInputElement ? 'input' : (node instanceof HTMLSelectElement ? 'change' : a.name + '-changed');
                        ...
                        if (event) {
                            for (let x in event.split(';')) {
                                if (node[x] instanceof TypedEvent)
                                    node[x].on((e) => this._bindingsSetValue(host ?? this, value.replaceAll('?', ''), node[camelCased], context));
                                else
                                    node.addEventListener(x, (e) => this._bindingsSetValue(host ?? this, value.replaceAll('?', ''), node[camelCased], context));
                            }
                        }
                    }
                }
    ...
    _textFragmentBinding(node, m, repeatBindingItems, removeAttributes, host, context) {
        ...
        const b = (firstRun) => this._bindingSetNodeValue(firstRun, node, null, 'innerHTML', value, repeatBindingItems, removeAttributes, host, context, noNull);
        this._bindings.push([b, null]);
        b(true);
    }
    ...
    _bindingsSetValue(obj, path, value, context) {
