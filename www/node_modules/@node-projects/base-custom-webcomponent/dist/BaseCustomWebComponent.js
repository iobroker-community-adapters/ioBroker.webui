...
export class BaseCustomWebComponentNoAttachedTemplate extends HTMLElement {
    ...
    _bindingsInternalParse(node, repeatBindingItems, removeAttributes, host, context) {
        if (!this._bindings)
            this._bindings = [];
        if (!node)
        ...
        if (node instanceof Element) { //node.nodeType === 1
            ...
            for (let a of attributes) {
                if (a.name.startsWith('css:') && a.value.startsWith('[[') && a.value.endsWith(']]')) {
                    ...
                    this._bindings.push(() => this._bindingSetElementCssValue(node, camelCased, value, repeatBindingItems, host, context));
                    ...
                    this._bindings.push(() => this._bindingSetElementClass(node, camelCased, value, repeatBindingItems, host, context));
                    this._bindings[this._bindings.length - 1](true);
                }
                    ...
                    this._bindings.push(() => this._bindingRepeat(node, bindingItemVariableName, bindingIndexname, value, changeItemCallback, repeatBindingItems, elementsCache, host, context));
                    this._bindings[this._bindings.length - 1](true);
                }
                    ...
                    this._bindings.push((firstRun) => this._bindingSetNodeValue(firstRun, node, a, camelCased, value, repeatBindingItems, removeAttributes, host, context, noNull));
                    this._bindings[this._bindings.length - 1](true);
                }
                else if (a.value.startsWith('{{') && a.value.endsWith('}}')) {
                    ...
                    this._bindings[this._bindings.length - 1](true);
                    if (event) {
                        event.split(';').forEach(x => node.addEventListener(x, (e) => this._bindingsSetValue(this, value, node[camelCased])));
                    }
                }
            }
        }
            ...
            if (node.nodeValue.indexOf('[[') >= 0) {
                ...
                const matches = text.matchAll(this.constructor._bindingRegex);
                let lastindex = 0;
                let fragment;
                for (let m of matches) {
                    if (m.index == 0 && m[0].length == text.length && node.parentNode.childNodes.length == 1) {
                        ...
                        this._bindings.push((firstRun) => this._bindingSetNodeValue(firstRun, parent, null, 'innerHTML', value, repeatBindingItems, removeAttributes, host, context, false));
                        this._bindings[this._bindings.length - 1](true);
                    }
                    else {
                        if (!fragment)
                        ...
                        const value = m[0].substr(2, m[0].length - 4);
                        this._bindings.push((firstRun) => this._bindingSetNodeValue(firstRun, newNode, null, 'innerHTML', value, repeatBindingItems, removeAttributes, host, context, false));
                        this._bindings[this._bindings.length - 1](true);
    ...
    _bindingSetNodeValue(firstRun, node, attribute, property, expression, repeatBindingItems, removeAttributes, host, context, noNull) {
        try {
            ...
            if (firstRun || node[property] !== value) {
                ...
                if (property === 'innerHTML' && value instanceof Element) {
                    ...
                    if (property[0] == '$') {
                        if (!value && noNull)
                            node.setAttribute(property.substring(1, property.length), '');
                        else
                            node.setAttribute(property.substring(1, property.length), value);
                    }
    ...
    _bindingsRefresh(property) {
        if (this._bindings)
            this._bindings.forEach(x => x(false));
    }
    _bindingsSetValue(obj, path, value) {
