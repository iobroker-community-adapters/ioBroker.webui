...
export function getBoxQuads(node, options) {
    ...
    if ((node instanceof Text || node instanceof _Text)) {
        ...
        const convertViewportRectToRelativeQuad = (rect) => {
            const rectInViewportRoot = new DOMRect(rect.x - (viewportRootRect?.x ?? 0), rect.y - (viewportRootRect?.y ?? 0), rect.width, rect.height);
        ...
        if (clientRects.length > 1) {
            ...
            for (const cr of clientRects) {
                ...
                quads.push(new DOMQuad(M_parent.transformPoint(new DOMPoint(lx, ly)), M_parent.transformPoint(new DOMPoint(lx + tw, ly)), M_parent.transformPoint(new DOMPoint(lx + tw, ly + th)), M_parent.transformPoint(new DOMPoint(lx, ly + th))));
            }
            if (quads.length > 0) {
                if (boxQuadsCache)
                    boxQuadsCache.set(key, quads);
    ...
    if ((node instanceof SVGGraphicsElement || node instanceof _SVGGraphicsElement)
        ...
        if (canUseViewportSvgRect) {
            ...
            if (needsStrokeInflation) {
                ...
                rect = new DOMRect(rect.x - strokeInflation.left * scaleX, rect.y - strokeInflation.top * scaleY, rect.width + (strokeInflation.left + strokeInflation.right) * scaleX, rect.height + (strokeInflation.top + strokeInflation.bottom) * scaleY);
            }
            let svgQuad;
            if (relativeTo === node.ownerDocument.body || relativeTo === node.ownerDocument.documentElement) {
                ...
                svgQuad = [new DOMQuad(new DOMPoint(relativeX, relativeY), new DOMPoint(relativeX + rect.width, relativeY), new DOMPoint(relativeX + rect.width, relativeY + rect.height), new DOMPoint(relativeX, relativeY + rect.height))];
            }
            else {
                const viewportRootRect = viewportRoot?.getBoundingClientRect();
                const rectInViewportRoot = new DOMRect(rect.x - (viewportRootRect?.x ?? 0), rect.y - (viewportRootRect?.y ?? 0), rect.width, rect.height);
...
export function getResultingTransformationBetweenElementAndAllAncestors(node, ancestor, iframes, excludeSelfZoom = false) {
    ...
    const useOwnSvgCtm = (actualElement instanceof SVGGraphicsElement || actualElement instanceof (actualElement.ownerDocument.defaultView ?? window).SVGGraphicsElement) &&
