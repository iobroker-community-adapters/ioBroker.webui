...
export class NpmPackageLoader {
    ...
    static addExportsToImportMap(imports, baseUrl, packageJsonObj) {
        const normalizedExportPath = (path) => baseUrl + removeLeading(removeLeading(path, '.'), '/');
        const addExport = (specifier, exportEntry) => {
    ...
    async getRegisteredElementTags(serviceContainer) {
        const tags = new Set();
    ...
    async loadDependencyCustomElementsJson(dependencies, serviceContainer, registeredTags) {
    ...
    async loadPackageCustomElementsJson(pkg, serviceContainer, registeredTags) {
        const addedTags = new Set();
    ...
    async loadNpmPackage(pkg, serviceContainer, paletteTree, loadAllImports, reportState) {
        ...
        if (customElementsJson.ok) {
            ...
            const registeredTags = serviceContainer ? await this.getRegisteredElementTags(serviceContainer) : new Set();
        ...
        let retVal = {};
        if (packageHack?.html) {
            retVal.html = packageHack.html.replaceAll("${baseUrl}", baseUrl);
        }
        if (packageHack?.style) {
            retVal.style = packageHack.style.replaceAll("${baseUrl}", baseUrl);
        }
        return retVal;
    }
...
//# sourceMappingURL=NpmPackageLoader.js.map