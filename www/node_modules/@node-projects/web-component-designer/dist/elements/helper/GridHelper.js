...
export function getGridLocalPoint(designItem, point) {
...
export function getElementLocalToCanvasMatrix(designItem) {
    const designerCanvas = designItem.instanceServiceContainer.designerCanvas;
...
export function getGridColumnIndexFromLocalX(gridInformation, localX) {
...
export function getGridColumnStartLineFromLocalX(gridInformation, localX) {
    const columns = gridInformation.cells[0];
    if (!columns?.length || !Number.isFinite(localX))
        return 1;
    let line = 1;
...
export function getGridRowIndexFromLocalY(gridInformation, localY) {
...
export function getGridRowStartLineFromLocalY(gridInformation, localY) {
    if (!gridInformation.cells.length || !Number.isFinite(localY))
        return 1;
    let line = 1;
    for (let i = 0; i < gridInformation.cells.length; i++) {
...
export function getGridCellFromPoint(designItem, point, gridInformation = calculateGridInformation(designItem)) {
...
export function calculateGridInformation(designItem) {
    ...
    const designerCanvas = designItem.instanceServiceContainer.designerCanvas;
    const transformedCornerPoints = designItem.element.getBoxQuads({ relativeTo: designerCanvas.canvas, iframes: designerCanvas.iframes })[0];
