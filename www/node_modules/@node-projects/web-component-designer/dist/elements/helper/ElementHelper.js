...
export function getElementOffsetParent(element) {
...
export function getElementOffsetsInContainer(element) {
...
export function getBoundingClientRectAlsoForDisplayContents(element) {
        ...
        if (cs.display == 'contents') {
            if (element.shadowRoot) {
                for (let c of element.shadowRoot.children) {
                    ...
                    r = new DOMRect(Math.min(r.x, rc.x), Math.min(r.y, rc.y), Math.max(r.width, rc.width), Math.max(r.height, rc.height));
                }
            }
            else {
...
export function getElementsWindowOffsetWithoutSelfAndParentTransformations(element, zoom, cache = {}) {
    ...
    while (currentElement) {
        ...
        let nextParent = currentElement.offsetParent ? currentElement.offsetParent : currentElement.getRootNode().host;
        if (instanceOfAny(currentElement, SVGSVGElement, HTMLBodyElement, HTMLHtmlElement)) {
            nextParent = currentElement.parentElement ? currentElement.parentElement : currentElement.getRootNode().host;
        }
        ...
        if (instanceOfAny(currentElement, SVGSVGElement, MathMLElement)) {
            ...
            const bcPar = currentElement.parentElement ? currentElement.parentElement.getBoundingClientRect() : currentElement.getRootNode().host.getBoundingClientRect();
