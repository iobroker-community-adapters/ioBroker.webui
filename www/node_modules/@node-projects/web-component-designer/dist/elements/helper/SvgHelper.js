...
import { createPathD } from './PathDataPolyfill.js';
...
export function isSupportedSvgGeometryElement(element) {
...
export function toOverlayPointFromSvgUserSpace(element, designerCanvas, point) {
...
export function fromOverlayPointToSvgUserSpace(element, designerCanvas, point) {
    const transform = _getSvgOverlayTransform(element, designerCanvas);
    if (transform) {
...
export function createOverlayPathDataFromSvgGeometryElement(element, designerCanvas, options) {
...
function _createOverlayPathDataWithoutMarkers(element, designerCanvas) {
    if (!isSupportedSvgGeometryElement(element)) {
        return null;
    }
    const getPathData = element.getPathData;
    if (!getPathData) {
        return null;
    }
    ...
    for (const command of sourcePathData) {
        ...
        switch (type) {
            case 'M': {
                ...
                transformedPathData.push({ type: 'M', values: [overlayPoint.x, overlayPoint.y] });
            ...
            case 'L': {
                const overlayPoint = toOverlay(values[0], values[1]);
                transformedPathData.push({ type: 'L', values: [overlayPoint.x, overlayPoint.y] });
                currentPoint = toAbsolute(values[0], values[1]);
                break;
            }
            case 'H': {
            ...
            case 'C': {
                ...
                transformedPathData.push({ type: 'C', values: [cp1.x, cp1.y, cp2.x, cp2.y, end.x, end.y] });
            ...
            case 'S': {
                ...
                transformedPathData.push({ type: 'S', values: [cp2.x, cp2.y, end.x, end.y] });
            ...
            case 'Q': {
                const cp1 = toOverlay(values[0], values[1]);
                const end = toOverlay(values[2], values[3]);
                transformedPathData.push({ type: 'Q', values: [cp1.x, cp1.y, end.x, end.y] });
                currentPoint = toAbsolute(values[2], values[3]);
                break;
            }
            case 'T': {
                const end = toOverlay(values[0], values[1]);
                transformedPathData.push({ type: 'T', values: [end.x, end.y] });
                currentPoint = toAbsolute(values[0], values[1]);
                break;
            }
            case 'A': {
                const end = toOverlay(values[5], values[6]);
                transformedPathData.push({ type: 'A', values: [values[0], values[1], values[2], values[3], values[4], end.x, end.y] });
                currentPoint = toAbsolute(values[5], values[6]);
                break;
            }
            case 'Z': {
                transformedPathData.push({ type: 'Z', values: [] });
...
function _createMarkerOverlayPathData(element, designerCanvas) {
...
function _collectMarkerPlacements(element) {
...
function _getNormalizedPathSegments(element) {
    const getPathData = element.getPathData;
    if (!getPathData) {
        return [];
    }
...
function _resolveMarkerReference(element, attributeName) {
...
function _createMarkerInstanceGroup(sourceElement, placement) {
...
function _getMarkerUnitsScale(sourceElement, marker) {
...
function _createViewBoxTransform(marker) {
...
function _mapMarkerPointToViewport(marker, viewBoxTransform, point) {
    if (!viewBoxTransform) {
        return point;
    }
    return {
...
function _getAlignFactors(align) {
    switch (align) {
...
function _resolveMarkerAngle(marker, incomingAngle, outgoingAngle, isStartMarker) {
...
function _bisectAngles(angleA, angleB) {
...
function _getFirstUsableAngle(points) {
...
function _getLastUsableAngle(points) {
...
function _angleBetween(start, end) {
...
function _getAnimatedLengthValue(length) {
    return length?.baseVal?.value ?? 0;
}
function _toElementReferenceBoxPoint(element, point) {
    const bbox = _getSvgGeometryBBoxOffset(element);
    return { x: point.x - bbox.x, y: point.y - bbox.y };
}
function _fromElementReferenceBoxPoint(element, point) {
    const bbox = _getSvgGeometryBBoxOffset(element);
    return { x: point.x + bbox.x, y: point.y + bbox.y };
}
function _getSvgGeometryBBoxOffset(element) {
...
function _getSvgOverlayTransform(element, designerCanvas) {
    if (!(element instanceof SVGGraphicsElement) || element instanceof SVGSVGElement) {
        return null;
    }
    const bbox = element.getBBox();
...
function _createProjectiveMatrixForQuad(quad) {
    const x1 = quad.p1.x;
    const y1 = quad.p1.y;
    const x2 = quad.p2.x;
    const y2 = quad.p2.y;
...
function _applyProjectiveMatrix(matrix, x, y) {
...
function _invertProjectiveMatrix(matrix) {
