export class CssCombiner {
    ...
    static applyStylesToHelper(styles) {
    ...
    static combineBrowserSupportedShorthands(styles) {
    ...
    static getNormalizedStyleValues(styles) {
    ...
    static getCandidateCoverage(styles, originalNormalizedValues, candidateName, candidateValue) {
    ...
    static matchesNormalizedStyleValues(styles, originalNormalizedValues) {
        const element = CssCombiner.applyStylesToHelper(styles);
    ...
    static readStyleValue(style, name) {
        if (name.startsWith('--') || name.includes('-'))
            return style.getPropertyValue(name).trim();
        return String(style[name] ?? '').trim();
    }
    static getSerializedSize(styles) {
        let size = 0;
        for (let style of styles)
            size += style[0].length + style[1].length + 2;
        return size;
    }
    static parseStyleDeclarationList(cssText) {
        let styles = new Map();
        let start = 0;
        let quote = null;
    ...
    static addStyleDeclaration(styles, declaration) {
...
//# sourceMappingURL=CssCombiner.js.map