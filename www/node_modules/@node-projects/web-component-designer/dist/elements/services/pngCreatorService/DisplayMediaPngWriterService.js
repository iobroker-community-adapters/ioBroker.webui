...
export class DisplayMediaPngWriterService {
    async takePng(designItems, options) {
        ...
        try {
            ...
            designerCanvas.disableBackgroud();
            ...
            const viewportElement = designerCanvas.outercanvas2;
            ...
            for (let iy = 0; iy < numTilesY; iy++) {
                for (let ix = 0; ix < numTilesX; ix++) {
                    ...
                    sleepTime = 300;
                    const dataUrl = await Screenshot.takeScreenshot(viewportElement, captureW, captureH);
                    const img = await this._loadImage(dataUrl);
                    // Draw only the inner region, skipping the 1px border on all sides
                    finalCtx.drawImage(img, insetPx, insetPx, effectiveCaptureW, effectiveCaptureH, ix * effectiveCaptureW, iy * effectiveCaptureH, effectiveCaptureW, effectiveCaptureH);
                }
            }
            ...
            designerCanvas.enableBackground();
            if (options?.removeSelection) {
...
//# sourceMappingURL=DisplayMediaPngWriterService.js.map