...
export class PropertyGridPropertyList extends BaseCustomWebComponentLazyAppend {
    ...
    _shouldCreateElements(designItem) {
    ...
    _rebuildElements(designItem, properties) {
        ...
        if (properties?.length) {
            for (let p of properties) {
                if ('properties' in p)
                    this.createPropertyGroups(p);
                else
                    this.createPropertyEditors(p);
            }
            return true;
        }
        return false;
    }
    createPropertyGroups(group) {
    ...
    expandOrCollapsePropertyGroups(propertyGroup) {
        for (let p of propertyGroup.properties) {
            ...
            if (property.editor.element)
                property.editor.element.style.display = displayStyle;
    ...
    createPropertyEditors(property, isInGroup) {
        let editor;
        let labelHolder;
        ...
        if (editor) {
            ...
            if (property.hideLabel)
                editor.element.style.gridColumn = '2 / 4';
