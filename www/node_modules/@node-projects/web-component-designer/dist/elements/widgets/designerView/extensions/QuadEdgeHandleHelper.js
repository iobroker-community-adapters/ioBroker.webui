...
export function getQuadCenter(quad) {
    return {
        x: (quad.p1.x + quad.p2.x + quad.p3.x + quad.p4.x) / 4,
        y: (quad.p1.y + quad.p2.y + quad.p3.y + quad.p4.y) / 4
    };
}
export function getEdgeMidpoint(start, end) {
    return {
        x: (start.x + end.x) / 2,
        y: (start.y + end.y) / 2
    };
}
export function getOutwardNormal(start, end, quadCenter, fallback) {
...
export function getEdgeOffsetPoint(start, end, quadCenter, distance, fallback) {
    const midpoint = getEdgeMidpoint(start, end);
...
//# sourceMappingURL=QuadEdgeHandleHelper.js.map