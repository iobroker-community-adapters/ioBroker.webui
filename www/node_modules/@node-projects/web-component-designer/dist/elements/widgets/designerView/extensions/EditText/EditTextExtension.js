  ...
  `;
    ...
    _getToolbarStateElement() {
        ...
        if (element && this.extendedItem.element.contains(element))
            return element;
        return this.extendedItem.element;
    }
    _isComputedFormatApplied(computedStyle, type, value) {
        switch (type) {
            case 'font-weight': {
    ...
    _getToolbarSelect(command) {
        return this._toolbar.querySelector(`select[data-command="${command}"]`);
    }
    _setToolbarButtonState(command, parameter, active) {
        const button = this._toolbar.querySelector(`button[data-command="${command}"][data-command-parameter="${parameter}"]`);
        if (!button)
            return;
    ...
    _setToolbarSelectValue(select, value) {
        if (!select)
            return;
    ...
    _setToolbarFontFamilyValue(select, value) {
        if (!select)
            return;
    ...
    _isFormatApplied(selection, type, value) {
    ...
    _getFirstSelectedElement(selection) {
    ...
    _findFormatTargets(span, type, value) {
        const targets = [];
        let current = span;
    ...
    _hasInlineFormat(element, type, value) {
        switch (type) {
            case 'font-weight':
    ...
    _unsetFormat(element, type, value) {
        switch (type) {
            case 'font-weight':
    ...
    _getTextDecorationValues(value) {
        if (!value)
            return [];
