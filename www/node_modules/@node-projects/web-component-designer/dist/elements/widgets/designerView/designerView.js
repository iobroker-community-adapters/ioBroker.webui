...
export class DesignerView extends BaseCustomWebComponentConstructorAppend {
    ...
    _lowertoolbar;
    _toolbar;
    _zoomHoldRepeater = new ZoomHoldRepeater();
    _zoomPointerId;
    _zoomPointerTarget;
    static style = css `
    :host {
    ...
    * {
    ...
    _zoomAroundViewportCenter(newZoom) {
        if (!Number.isFinite(newZoom) || newZoom <= 0)
            return;
        const viewport = this._designerCanvas.outerRect;
        const centerPoint = getCanvasPointAtViewportCenter(viewport.width, viewport.height, this._designerCanvas.zoomFactor, this._designerCanvas.canvasOffset);
        this._designerCanvas.zoomPoint(centerPoint, newZoom);
    }
    _configureZoomButton(button, direction) {
        button.onpointerdown = event => {
            ...
            this._zoomHoldRepeater.start(() => this._zoomStep(direction), () => this._zoomStep(direction, 4));
    ...
    _zoomStep(direction, stepDivisor = 1) {
