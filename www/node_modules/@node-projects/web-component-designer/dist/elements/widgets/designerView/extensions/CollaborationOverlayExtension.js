...
import { getCollaborationNodeIndex } from '../../../services/collaborationService/CollaborationNodeIndex.js';
const collaborationOverlayCacheKey = Symbol('collaborationOverlayCache');
function getOverlayCache(designItem, cache) {
...
export class CollaborationOverlayExtension extends AbstractExtension {
    ...
    refresh(cache, event) {
        ...
        peers.forEach((peer, index) => {
            ...
            const peerRect = this._drawRect(rect.x - offset, rect.y - offset, rect.width + (offset * 2), rect.height + (offset * 2), 'svg-collaboration-peer', null, OverlayLayer.Foreground);
            ...
            this._drawTextWithBackground(labelText, rect.x - offset, rect.y - 8 - (index * 14), peer.color ?? '#1f7ae0', 'svg-collaboration-label', null, OverlayLayer.Foreground);
        });
        if (comments.length) {
            const unresolvedComments = comments.filter(x => !x.resolved);
            const badgeColor = unresolvedComments.length ? '#f57c00' : '#9e9e9e';
            const commentRect = this._drawRect(rect.x + 2, rect.y + 2, Math.max(0, rect.width - 4), Math.max(0, rect.height - 4), 'svg-collaboration-comment', null, OverlayLayer.Foreground);
            ...
            this._drawTextWithBackground(commentLabel, rect.x + rect.width - 12, rect.y + 14, badgeColor, 'svg-collaboration-comment-label', null, OverlayLayer.Foreground);
            const previewComment = comments[comments.length - 1];
            this._drawTextWithBackground(truncateText(previewComment.text, 28), rect.x + 4, rect.y + rect.height + 16, badgeColor, 'svg-collaboration-comment-preview', null, OverlayLayer.Foreground);
        }
    }
    dispose() {
        this._removeAllOverlays();
...
//# sourceMappingURL=CollaborationOverlayExtension.js.map