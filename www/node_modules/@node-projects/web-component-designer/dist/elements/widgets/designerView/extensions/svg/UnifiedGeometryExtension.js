...
import { SegmentType } from './geometry/IGeometry.js';
...
export class UnifiedGeometryExtension extends AbstractExtension {
    ...
    _overlayLines = [];
    constructor(extensionManager, designerView, extendedItem, reader, options) {
    ...
    _drawAnchorCircle(overlayPt, segIndex, seg) {
        ...
        const onDown = (e) => this._onPointerDown(e, circle, segIndex, 'anchor');
        const onMove = (e) => this._onPointerMove(e, circle, segIndex, 'anchor');
        const onUp = (e) => this._onPointerUp(e, segIndex);
        const onCtx = (e) => {
            e.preventDefault();
    ...
    _drawControlCircle(overlayPt, segIndex, handleType) {
        ...
        const onDown = (e) => this._onPointerDown(e, circle, segIndex, handleType);
        const onMove = (e) => this._onPointerMove(e, circle, segIndex, handleType);
        const onUp = (e) => this._onPointerUp(e, segIndex);
        this._addTrackedListener(circle, EventNames.PointerDown, onDown);
        this._addTrackedListener(circle, EventNames.PointerMove, onMove);
        this._addTrackedListener(circle, EventNames.PointerUp, onUp);
    ...
    _onPointerDown(event, circle, segIndex, handleType) {
    ...
    _selectSourcePart(segIndex, handleType, event) {
    ...
    _getSourcePart(segIndex, handleType) {
    ...
    _isSelectedHandle(circleKey) {
        const [handleType, segmentIndexText] = circleKey.split('-');
        const selectedPart = this.extendedItem.instanceServiceContainer.selectionService.selectedPart;
        const data = selectedPart?.data;
    ...
    _onPointerMove(event, circle, segIndex, handleType) {
        event.stopPropagation();
        event.preventDefault();
        if (!this._dragState || !this._geometry || event.buttons === 0)
            return;
        if (this._dragState.segmentIndex !== segIndex || this._dragState.handleType !== handleType)
            return;
        const cursorPos = this.designerCanvas.getNormalizedEventCoordinates(event);
    ...
    _dragSingleHandle(segIndex, handleType, dx, dy) {
        const origOverlay = this._toOverlay(this._dragState.originalPoint);
        const newOverlay = { x: origOverlay.x + dx, y: origOverlay.y + dy };
    ...
    _dragWholeShape(dx, dy) {
        ...
        for (let i = 0; i < this._geometry.segments.length; i++) {
            ...
            if (seg.cp1 && orig.cp1) {
                ...
                const newCp1Overlay = { x: origCp1Overlay.x + dx, y: origCp1Overlay.y + dy };
            ...
            if (seg.cp2 && orig.cp2) {
                ...
                const newCp2Overlay = { x: origCp2Overlay.x + dx, y: origCp2Overlay.y + dy };
    ...
    _applyGeometryToElement() {
        if (!this._reader || !this._geometry)
            return;
    ...
    // -- Context menu --
    _showContextMenu(event, segIndex, seg) {
        if (!this._geometry)
            return;
        const items = [];
    ...
    _findLastNonCloseIndex() {
        if (!this._geometry)
            return -1;
        for (let i = this._geometry.segments.length - 1; i >= 0; i--) {
            if (this._geometry.segments[i].type !== SegmentType.Close)
                return i;
        }
        return -1;
    }
    _findNextNonCloseSegment(segIndex) {
        if (!this._geometry)
            return null;
    ...
    _insertPointAfter(segIndex) {
        ...
        switch (nextSeg.type) {
            case SegmentType.CubicBezier: {
                ...
                const p0 = seg.point;
                const p1 = nextSeg.cp1;
                const p2 = nextSeg.cp2;
    ...
    _convertToHorizontalLine(segIndex) {
        if (!this._geometry)
            return;
        const seg = this._geometry.segments[segIndex];
        const prevPt = this._findPreviousAnchor(segIndex);
    ...
    _convertToVerticalLine(segIndex) {
        if (!this._geometry)
            return;
        const seg = this._geometry.segments[segIndex];
        const prevPt = this._findPreviousAnchor(segIndex);
    ...
    _evalQuadraticBezier(p0, p1, p2, t) {
        const mt = 1 - t;
        return {
...
//# sourceMappingURL=UnifiedGeometryExtension.js.map