...
export class SvgPathGeometryReader {
    read(element) {
        const pathEl = element;
        const pathData = pathEl.getPathData({ normalize: false });
        const segments = [];
        let lastPos = { x: 0, y: 0 };
        ...
        for (const seg of geometry.segments) {
            ...
            switch (seg.type) {
                case SegmentType.Move: {
                    ...
                    pathData.push({ type: rel ? 'm' : 'M', values: [x, y] });
                    lastPos = { ...seg.point };
                    break;
                }
                case SegmentType.Line: {
                    const x = rel ? seg.point.x - lastPos.x : seg.point.x;
                    const y = rel ? seg.point.y - lastPos.y : seg.point.y;
                    pathData.push({ type: rel ? 'l' : 'L', values: [x, y] });
                    lastPos = { ...seg.point };
                    break;
                }
                case SegmentType.HorizontalLine: {
                    const x = rel ? seg.point.x - lastPos.x : seg.point.x;
                    pathData.push({ type: rel ? 'h' : 'H', values: [x] });
                    lastPos = { ...seg.point };
                    break;
                }
                case SegmentType.VerticalLine: {
                    const y = rel ? seg.point.y - lastPos.y : seg.point.y;
                    pathData.push({ type: rel ? 'v' : 'V', values: [y] });
                    lastPos = { ...seg.point };
                    break;
                }
                case SegmentType.CubicBezier: {
                    ...
                    const x = rel ? seg.point.x - lastPos.x : seg.point.x;
                    const y = rel ? seg.point.y - lastPos.y : seg.point.y;
                    pathData.push({ type: rel ? 'c' : 'C', values: [cp1x, cp1y, cp2x, cp2y, x, y] });
                    lastPos = { ...seg.point };
                    break;
                }
                case SegmentType.SmoothCubic: {
                    ...
                    const x = rel ? seg.point.x - lastPos.x : seg.point.x;
                    const y = rel ? seg.point.y - lastPos.y : seg.point.y;
                    pathData.push({ type: rel ? 's' : 'S', values: [cp2x, cp2y, x, y] });
                    lastPos = { ...seg.point };
                    break;
                }
                case SegmentType.QuadraticBezier: {
                    ...
                    const x = rel ? seg.point.x - lastPos.x : seg.point.x;
                    const y = rel ? seg.point.y - lastPos.y : seg.point.y;
                    pathData.push({ type: rel ? 'q' : 'Q', values: [cp1x, cp1y, x, y] });
                    lastPos = { ...seg.point };
                    break;
                }
                case SegmentType.SmoothQuadratic: {
                    const x = rel ? seg.point.x - lastPos.x : seg.point.x;
                    const y = rel ? seg.point.y - lastPos.y : seg.point.y;
                    pathData.push({ type: rel ? 't' : 'T', values: [x, y] });
                    lastPos = { ...seg.point };
                    break;
                }
                ...
                case SegmentType.Close: {
                    pathData.push({ type: rel ? 'z' : 'Z', values: [] });
                    break;
                }
            }
        }
...
//# sourceMappingURL=SvgPathGeometryReader.js.map