...
function parseOneShadowLayer(token) {
...
export class BoxShadowEditorWindow extends DraggableToolWindow {
    ...
    _copyBtn;
    _applyBtn;
    get windowTitle() { return 'Box Shadow Editor'; }
    ...
    constructor(designerCanvas) {
        ...
        this._layerList = this._getDomElement('bse-layer-list');
        this._removeBtn = this._getDomElement('bse-remove-btn');
        this._insetCheck = this._getDomElement('bse-inset');
        this._enabledCheck = this._getDomElement('bse-enabled');
        this._inputX = this._getDomElement('bse-x');
        this._inputY = this._getDomElement('bse-y');
        this._inputBlur = this._getDomElement('bse-blur');
        this._inputSpread = this._getDomElement('bse-spread');
        this._inputColor = this._getDomElement('bse-color');
        this._inputOpacity = this._getDomElement('bse-opacity');
        this._opacityVal = this._getDomElement('bse-opacity-val');
        this._previewBox = this._getDomElement('bse-preview');
        this._cssOutput = this._getDomElement('bse-css-out');
        this._loadBtn = this._getDomElement('bse-load-btn');
        this._copyBtn = this._getDomElement('bse-copy-btn');
        this._applyBtn = this._getDomElement('bse-apply-btn');
        this._getDomElement('bse-add-btn').onclick = () => this._addLayer();
        this._getDomElement('bse-dup-btn').onclick = () => this._duplicateLayer();
        this._getDomElement('bse-up-btn').onclick = () => this._moveLayer(-1);
        this._getDomElement('bse-down-btn').onclick = () => this._moveLayer(1);
        ...
        for (const [input, key, valId] of [
            ...
            input.oninput = () => {
                this._currentLayer()[key] = Number(input.value);
                this._getDomElement(valId).textContent = input.value + 'px';
                this._refresh();
            };
        }
    ...
    _loadControls() {
        ...
        this._getDomElement('bse-x-val').textContent = l.offsetX + 'px';
        this._inputY.value = String(l.offsetY);
        this._getDomElement('bse-y-val').textContent = l.offsetY + 'px';
        this._inputBlur.value = String(l.blur);
        this._getDomElement('bse-blur-val').textContent = l.blur + 'px';
        this._inputSpread.value = String(l.spread);
        this._getDomElement('bse-spread-val').textContent = l.spread + 'px';
...
//# sourceMappingURL=BoxShadowEditorWindow.js.map