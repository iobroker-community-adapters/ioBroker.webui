import { BaseCustomWebcomponentBindingsService, BindingTarget, EventsService, JsonFileElementsService, PreDefinedElementsService, PropertiesHelper, SeperatorContextMenu, TextRefactorService, createDefaultServiceContainer } from "@node-projects/web-component-designer";
...
export function configureDesigner(bindingsHelper) {
    ...
    serviceContainer.config.openBindingsEditor = async (property, designItems, binding, target) => {
        if (!binding || binding.service instanceof VisualizationBindingsService) {
            ...
            let dynEdt = new IobrokerWebuiBindingsEditor(property, binding, target, serviceContainer, designItems[0].instanceServiceContainer, window.appShell, existingBinding?.[1].type);
