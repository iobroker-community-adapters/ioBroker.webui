...
import { IElementDefinition, dragDropFormatNameElementDefinition, dragDropFormatNamePropertyGrid } from '@node-projects/web-component-designer';
...
export class IobrokerWebuiIconsView extends BaseCustomWebComponentConstructorAppend {
    ...
    static readonly is = 'iobroker-webui-icons-view';
    ...
    async ready() {
        ...
        for await (const f of this.iconNames()) {
            ...
            div.ondragstart = (event) => {
                const elementDef: IElementDefinition = { tag: "img", defaultAttributes: { 'src': src } }
    ...
    async* iconNames(): AsyncGenerator<{ name: string, path: string }, void, unknown> {
    ...
    async* _readFolder(adapter: string, subFolder: string): AsyncGenerator<{ name: string, path: string }, void, unknown> {
