import { IDesignerCanvas, IBindableObject, DesignItem, BindingTarget, InsertAction } from "@node-projects/web-component-designer";
import { BindableObjectDragDropService, VisualizationBinding } from "@node-projects/web-component-designer-visualization-addons";

export class IobrokerWebuiBindableObjectDragDropService extends BindableObjectDragDropService {
    override async drop(designerCanvas: IDesignerCanvas, event: DragEvent, bindableObject: IBindableObject<any>, element: Element): Promise<void> {
        ...
        if (element == null && bindableObject.name == 'url' && bindableObject.fullName.startsWith('ring.') && bindableObject.fullName.endsWith('Snapshot.url')) {
            ...
            const binding: VisualizationBinding = { signal: bindableObject.fullName + ';' + sn2, target: BindingTarget.property };
            ...
            (<HTMLImageElement>di.element).src = <string>state.val;
