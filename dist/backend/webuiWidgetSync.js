...
function syncWidgetSets() {
    ...
    let widgetJs = `import { PreDefinedElementsService } from "../node_modules/@node-projects/web-component-designer/dist/elements/services/elementsService/PreDefinedElementsService.js";
...
`;
    ...
    for (let d = 0; d < sets.length; d++) {
        ...
        for (let i of sets[d].info.imports) {
            widgetJs += '    import(window.iobrokerWebuiRootUrl + "webui-widgets/' + i + '");\n';
            widgetLoader += '    import(window.iobrokerWebuiRootUrl + "webui-widgets/' + i + '");\n';
        }
        widgetJs += '    serviceContainer.register("elementsService", new PreDefinedElementsService("' + sets[d].info.name + '", { elements: [' + Object.keys(sets[d].info.widgets).map(x => '"' + x + '"').join(', ') + '] }));\n';
    }
    widgetJs += '}';
    let newConfig = JSON.stringify(widgetInfo);
    if (oldConfig !== newConfig) {
        fs.writeFileSync(__dirname + '/../www/webui-widgets/config.json', newConfig);
        fs.writeFileSync(__dirname + '/../www/webui-widgets/configWidgets.js', widgetJs);
        fs.writeFileSync(__dirname + '/../www/webui-widgets/configLoadWidgets.js', widgetLoader);
        return newConfig;
    }
    else {
        return false;
